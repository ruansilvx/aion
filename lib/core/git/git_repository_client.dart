...
class GitRepositoryClient {
  /// Returns whether [rootPath] is already inside a git working tree — checked
  /// via `<rootPath>/.git` existing, not a `git` subprocess call (cheap,
  /// consistent with `ProjectStackDetector`'s own marker-file-only approach).
  /// `.git` may be a directory (an ordinary repo) or a file (a linked
  /// worktree/submodule) — both count as "already a repo" for this check's
  /// purposes; it doesn't distinguish between them since neither should have
  /// `git init`/bookkeeping written over it. Added for `AIO-1266` — lets
  /// `CreateProjectCubit` skip re-`init`ing an already-git-tracked directory
  /// and gate its gitignore-confirmation banner.
  ...
  /// creating a new branch checked out into an isolated working directory at
  /// [worktreePath] — the branch starts from [rootPath]'s current HEAD. Throws
  /// if `worktreePath` already exists or `branchName` is already checked out
  /// elsewhere. Added for `AIO-506` — isolates a coding-execution run from the
  ...
  /// Runs `git branch -D <branchName>` in [rootPath] — force-deletes a local
  /// branch regardless of merge status, since a branch this deletes is by
  /// definition never going anywhere (an abandoned coding-execution attempt
  /// that never reached a real push). Callers must ensure no worktree still
  /// has [branchName] checked out first ([removeWorktree] before this) —
  /// `git branch -D` refuses otherwise. Added for `AIO-506`'s branch-naming
  /// scheme (`aion/task-<id>`, stable per task, never per attempt) combined
  /// with [removeWorktree] deliberately not deleting the branch itself:
  /// without this, a failed attempt's branch survived forever, and — since
  /// `TicketsCubit._runCodingExecution` reuses the exact same branch name on
  /// every retry of the same task — every subsequent retry's `createWorktree`
  /// call failed identically with "a branch named '...' already exists",
  /// confirmed via live reproduction.
  ...
  /// Runs `git diff --name-only <baseBranch>...<branch>` in [worktreePath] and
  /// returns the count of changed files. Computed locally rather than via
  /// `gh pr view --json files` (a second network call, with a risk of
  /// eventual-consistency lag immediately after `gh pr create` returns) — the
  /// worktree already has everything needed on disk. Called just before `push`
  /// in `TicketsCubit._runCodingExecution`, from inside the worktree `push`
  /// itself already runs from. Added for `AIO-1586`.
  ...
  /// resolved and doesn't require the worktree to exist yet) and strips the
  /// `origin/` prefix, returning e.g. `'main'`. Falls back to `'main'` if the
  /// command fails (a shallow clone or a repo with no `origin/HEAD` set) —
  /// matching `gh pr create`'s own base-branch resolution fallback
  /// (repository's configured default branch). Added for `AIO-1586`.
  ...
  /// `TicketsCubit.confirmRelease` can surface it and keep the draft alive for
  /// retry rather than reporting success it didn't earn. Added for `AIO-1782`.
  Future<void> tag(String rootPath, String tagName, String message) async {
    await _runChecked(['tag', '-a', tagName, '-m', message], rootPath);
  }

  /// Runs `git push origin <tagName>` in [rootPath] — pushes the tag [tag]
  /// already created locally. Uses [_runChecked], same rationale as [tag]:
  /// `TicketsCubit.confirmRelease` calls this only after the version-bump
  /// commit has already been pushed via [push], so a failure here means the
  /// tag exists locally but not on `origin` yet — [confirmRelease]'s own
  /// dartdoc covers the retry story for that partial-failure state. Added for
  /// `AIO-1782`.
