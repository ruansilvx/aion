...
/// other than `projects` that needs to know the active project depends only on
/// this interface — never on `features/projects/` directly. See `AIO-1174` §4.
abstract interface class ActiveProjectProvider {
  ...
  /// Whether the ticket list should show the opt-in codebase-summarization
  /// offer banner the next time it builds. `true` only immediately after
  /// [activeProject] was switched to with the "created from an
  /// already-git-tracked directory" flag set — in-memory only, not persisted.
  /// Call [consumeCodebaseAnalysisOffer] once the offer has been shown (or
  /// deliberately not shown) so it never reappears. Added for `AIO-1266`.
  ...
  /// Whether the ticket list should show the baseline-upgrade offer banner the
  /// next time it builds. `true` whenever [activeProject]'s pinned baseline
  /// version isn't the latest version bundled in the running build —
  /// recomputed fresh every time [activeProject] switches, never persisted, so
  /// declining only dismisses the current instance: it can reappear the next
  /// time this project is opened. Call [consumeBaselineUpgradeOffer] once the
  /// offer has been shown (or deliberately not shown) so it doesn't reappear
  /// within the same open. Added for `AIO-297`.
