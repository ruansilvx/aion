...
/// [TicketBoardColumnVisibility] as one project-id-prefixed string-list key
/// (`ticket_board_column_visibility.<projectId>.hiddenStatuses`), each entry a
/// project-defined `WorkflowStatus.name` — mirrors
/// `SharedPrefsTicketListFilterRepository`'s string-list convention. A stale
/// stored name (a status the project has since deleted or renamed) round-trips
/// through this repository unchanged; it's the caller
/// (`TicketsCubit`/`TicketBoardView`, which only ever render checkboxes for
/// the project's currently-configured statuses) that naturally never matches
/// it against anything live, so it's dropped the next time a fresh selection
/// is persisted rather than validated here — this repository performs no
/// validation at all, per this project's Cubit-vs-repository split (was a
/// `TicketStatus.values.where(...)` enum round-trip before `AIO-549`).
