// presentation/widgets/ticket_metadata_section.dart — TicketMetadataSection and its supporting private widgets (presentation layer).
...
class TicketMetadataSection extends StatelessWidget {
  ...
  /// comment for why this isn't read directly in [build].
  final Ticket ticket;

  /// The persisted SDD-stage-triggering [AutomationConfidence], as
  /// loaded once by the owning screen (`TicketDetailScreen.initState`)
  /// — passed down rather than re-fetched here so every
  /// [TicketMetadataSection] instance shares one load.
  final AutomationConfidence? automationConfidence;

  /// Called to advance [ticket]'s SDD stage on an explicit user tap
  /// (gated/manual automation) — see
  /// `TicketDetailScreen._advanceSddStage`.
  final void Function(Ticket ticket) onAdvanceSddStage;

  /// Called once per build for an `epic`/`story` [ticket] so the
  /// caller can auto-advance its SDD stage when
  /// [AutomationConfidence.auto] and `canAdvance` allow it — see
  /// `TicketDetailScreen._maybeAutoAdvanceSddStage`. Kept as a
  /// callback rather than inlined here because the auto-advance guard
  ...
  /// `TicketDetailScreen._tryReadRegistry`.
  ...
  /// Whether [ticket] should show the sync badge/repair banner — mirrors
  /// `TicketDetailScreen._isSyncable`.
  bool _isSyncable(BuildContext context, Ticket ticket) {
    if (ticket.type != TicketType.resource) return false;
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            InlineEditableField<String>(
                              ...
                              semanticsLabel:
                                  context.l10n.ticketDetailEditTitle,
                              parser: (raw) {
                            ...
                            if (ticket.type != TicketType.epic) ...[
                              ...
                              TicketParentPicker(
                                ...
                                onParentSelected: (parentId) => context
                                    .read<TicketsCubit>()
                                    .updateTicketParent(ticket, parentId),
                              ),
                            ],
                            const SizedBox(height: AionSpacing.sp12),
                            Row(
                            ...
                            if (needsDesignReview == true &&
                              ...
                              _LinkedDesignPageChip(
                                ...
                                onTap: () => context.go(
                                  ticketDetailRoute(linkedDesignPage),
                                ),
                              ),
                            ],
                            ...
                            if (ticket.type == TicketType.epic ||
                              ...
                              _SddStageSection(
                                ticket: ticket,
                                canAdvance: canAdvanceSddStage,
                                blockReason: sddStageBlockReason,
                            ...
                            InlineEditableField<String?>(
                              ...
                              semanticsLabel:
                                  context.l10n.ticketDetailEditDescription,
                              parser: (raw) {
                                final trimmed = raw.trim();
                                return trimmed.isEmpty ? null : trimmed;
                              },
                      ...
                      return all
                          .where(
                            (candidate) =>
                                ...
                                candidate.type != TicketType.page &&
                                candidate.type != TicketType.resource,
                          )
                          .toList();
                    },
...
class _StageTrackerRowState extends State<_StageTrackerRow> {
  ...
  void _jumpToCurrent() {
    ...
    final nodeCenter = widget.currentIndex * widget.cellW + widget.cellW / 2;
    final target =
        (nodeCenter - _scrollController.position.viewportDimension / 2).clamp(
          0.0,
...
class _ExecutionLiveToolLine extends StatefulWidget {
  ...
  @override
  State<_ExecutionLiveToolLine> createState() => _ExecutionLiveToolLineState();
}
...
class _ExecutionActionButton extends StatefulWidget {
  ...
  @override
  State<_ExecutionActionButton> createState() => _ExecutionActionButtonState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: FocusableActionDetector(
        ...
        child: MouseRegion(
          cursor: _isConfirming ? MouseCursor.defer : SystemMouseCursors.click,
