...
/// Read/write access to [WorkflowStatus] persistence. A dumb persistence layer
/// only — no validation, no invariant enforcement. Every domain invariant
/// (name uniqueness within scope, exactly-one-holder-per-role) lives in
...
/// by the data layer ([DriftWorkflowStatusRepository]); UI and domain code
/// depend only on this interface, never on a concrete data source. See
/// `AIO-549` §1.6.
abstract interface class WorkflowStatusRepository {
  ...
  /// `WorkflowConfigCubit`'s own state subscribe to, so the two Cubits stay
  /// consistent through the repository layer rather than holding a direct
  /// reference to each other — Aion's Cubits communicate through shared
  /// repositories, never each other directly. See `AIO-549` §3.
