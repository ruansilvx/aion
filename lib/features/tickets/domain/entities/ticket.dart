...
class Ticket extends Equatable {
  ...
  /// Generated by [TicketRepository.createTicket]; empty on entities not yet
  /// persisted.
  ...
  /// Current workflow state — a project-defined status name (e.g. `"backlog"`,
  /// `"in_progress"`, or a project's own renamed/added status), resolved
  /// against `WorkflowStatusRepository`/ `WorkflowConfigCubit` rather than a
  /// fixed enum. Prior to `AIO-549`, this field was a fixed `TicketStatus`
  /// enum; every project now configures its own status set (see
  /// `WorkflowStatus`), seeded by default with `defaultWorkflowStatuses` so an
  /// unconfigured project behaves exactly as before. `TicketsCubit._roleOf`
  /// resolves a status string to its configured `WorkflowStatusRole`, if any,
  /// for every place that used to gate on a literal `TicketStatus` value.
  final String status;
  ...
  /// Where the current [complexity] value came from, or `null` if [complexity]
  /// itself is unset. Written only by `TicketRepository`'s dedicated methods —
  /// see [complexity]'s dartdoc and `AIO-75` §1.2. Never part of [copyWith].
  ...
  /// prediction time) or the ticket has already started executing. Written
  /// only by `TicketTokenPredictor` via
  /// `TicketRepository.applyTokenPrediction` — never part of [copyWith], same
  /// treatment as [complexitySource]/[estimateSource] and for the same reason:
  /// a generic content edit must not be able to silently clobber a value only
  /// a dedicated write path should touch. See `AIO-66` for the failure mode
  /// this precedent avoids.
  ...
  /// zero-arg setter instead of a bare value — pass `() => null` to explicitly
  /// clear one of them, or omit the parameter entirely to leave it unchanged.
  /// A plain `?? this.x` fallback can't tell "not passed" apart from
  /// "explicitly set to null," since both look like `null` at the call site.
  /// `id`, `ticketId`, `parentId`, `embedding`, `syncStatus`, `createdAt`,
  /// `deletedAt`, `sddStage`, `estimateRollup`, `timeSpentRollup`,
  /// `complexitySource`, `estimateSource`, `predictedExecutionTokensLow`, and
  /// `predictedExecutionTokensHigh` are never mutated by this method —
  /// `deletedAt` is written only via
  /// `TicketRepository.trashTicket`/`trashTickets`/`restoreTicket`, `sddStage`
  /// is written only via `TicketsCubit.advanceSddStage`,
  /// `estimateRollup`/`timeSpentRollup` are written only via
  /// `TicketRepository.updateRollup`, `complexitySource`/`estimateSource` are
  /// written only via
  /// `TicketRepository.updateTicket`/`applyEstimationSuggestion`, and
  /// `predictedExecutionTokensLow`/`predictedExecutionTokensHigh` are written
  /// only via `TicketRepository.applyTokenPrediction`, so none of their
  /// preconditions can be bypassed by a plain edit. Like
  /// `estimateRollup`/`timeSpentRollup`, `deletedAt`/`complexitySource`/
  /// `estimateSource`/`predictedExecutionTokensLow`/
  /// `predictedExecutionTokensHigh` still pass through unchanged on every call
  /// (there's no settable parameter for any of them) — only
  /// [complexity]/[estimate] themselves are settable here. `deletedAt` used to
  /// be silently dropped (reset to `null`) here instead of passed through —
  /// see `AIO-66` for that past failure mode, now fixed by following this same
