...
/// One row in a doc's Backlinks section: another ticket that references this
/// one, either via an explicit `TicketLink` or an inline `[[wikilink]]`.
/// Replaces the bare `List<Ticket>`/`List<LinkedTicketRef>` `BacklinksSection`
/// previously took — a link's origin never survived past
/// `TicketsCubit.loadDocumentRelations`/`PageTicketProviderImpl .loadPageRelations`
/// before this, so nothing could render it differently. Per `AIO-963`.
