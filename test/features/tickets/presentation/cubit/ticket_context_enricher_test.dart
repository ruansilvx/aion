...
void main() {
  ...
  group('relatedTicketsSection', () {
    test(
      'returns "" for a ticket with no parent, no links, and no '
    ...
    test(
      'renders a multi-level ancestor chain nearest-first, each with its '
      'type label',
      () async {
        final epic = ticket(id: 'epic', type: TicketType.epic, title: 'Epic title');
        final story = ticket(
          id: 'story',
    ...
    test(
      ...
      () async {
        ...
        when(() => linkRepository.getLinksForTicket('target')).thenAnswer(
          (_) async => [
            TicketLinkData(
              id: 'link1',
              sourceTicketId: 'source',
        ...
        expect(fromSource, contains('- blocks — "Target ticket"'));
        expect(fromTarget, contains('- blockedBy — "Source ticket"'));
      },
    );

    test(
      'a similarity match below the threshold is excluded; one at or '
    ...
    test(
      'a ticket already surfaced by the structured walk is never '
    ...
    test('more than the top-K similarity matches above threshold are capped', () async {
      final source = ticket(id: 'source', title: 'Source ticket');
    ...
    test(
      'a description longer than the snippet cap is truncated with a '
